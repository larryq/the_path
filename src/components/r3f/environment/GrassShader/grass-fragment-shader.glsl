...
vec3 lambertLight(vec3 normal, vec3 viewDir, vec3 lightDir, vec3 lightColour) {
  float wrap = 0.5;
  float dotNL = saturateVal((dot(normal, lightDir) + wrap) / (1.0 + wrap));
...
vec3 phongSpecular(vec3 normal, vec3 lightDir, vec3 viewDir) {
  float dotNL = saturateVal(dot(normal, lightDir));
...
void main() {
  ...
  vec4 baseColour = texture(grassDiffuse, vec3(uv, grassType));
