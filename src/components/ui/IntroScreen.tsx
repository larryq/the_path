...
export default function IntroScreen() {
  ...
  return (
    <div
      ...
      style={{
        background: 'radial-gradient(ellipse at 50% 60%, #1a3a2a 0%, #0d1f15 60%, #060e0a 100%)',
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 px-6 max-w-xl text-center">
        ...
        <div className="flex flex-col items-center gap-2">
          <div className="text-xs tracking-[0.4em] uppercase mb-1"
            style={{ color: 'var(--color-accent)', fontFamily: "'Raleway', sans-serif", fontWeight: 300 }}>
            a nature walk
          </div>
          <h1 className="text-5xl md:text-6xl leading-tight"
            style={{ fontFamily: "'Cinzel', serif", fontWeight: 700,
              color: 'var(--color-text-light)', textShadow: '0 0 60px rgba(126,203,161,0.3)' }}>
            The Path
          </h1>
          <div className="w-16 h-px mt-2"
            style={{ background: 'linear-gradient(to right, transparent, var(--color-accent), transparent)' }} />
        </div>

        <div className="flex flex-col gap-4 text-sm leading-relaxed"
          style={{ color: 'var(--color-text-dim)', fontFamily: "'Raleway', sans-serif", fontWeight: 300 }}>
          <Row text="Click and drag across the hexagonal tiles to draw a winding path." />
          <Row text="Release to lock the path. Bordering tiles will glow — click to choose what grows there." />
        ...
        <button
          ...
          className="mt-2 px-10 py-4 text-sm tracking-[0.3em] uppercase transition-all duration-300"
          style={{
            fontFamily: "'Raleway', sans-serif", fontWeight: 500,
            color: 'var(--color-forest-deep)',
            background: 'linear-gradient(135deg, var(--color-accent) 0%, #5aad80 100%)',
...
function Row({ text }: { text: string }) {
  return (
    <div className="flex items-start gap-3 text-left">
      <span style={{ color: 'var(--color-accent)', flexShrink: 0, marginTop: '2px', fontSize: '0.6rem' }}>✦</span>
      <span>{text}</span>
    </div>
  )
}
